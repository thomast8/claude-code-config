...
## Review Negotiator
- Plugin `review-negotiator` gets a genuine second opinion or a full review-to-convergence from Codex via a shared local MCP server. Different from `codex:codex-rescue` (a one-shot forward-and-implement handoff): this is for when Codex should actually push back on or agree with a specific claim/diff/plan, with real iteration.
- `/review-negotiator:negotiate [code|plan] [focus text]` - full negotiation to convergence: Claude (proposer) and Codex (reviewer) go back and forth until Codex approves (`agreed`) or the round cap is hit with genuine disagreement still standing (`deadlocked` - a real, reported outcome, never a forced consensus).
- `/review-negotiator:consult [question]` - single-shot second opinion from Codex, usable mid-planning (closest analog to the built-in `advisor()` tool, but from Codex's side); promotable into a full negotiation if real disagreement surfaces.
- Natural language triggers it too (no proactive/unprompted use - only on explicit request).
- Requires exiting plan mode first (plan mode hard-blocks the MCP/Bash calls it needs) and a live interactive session (a one-shot headless `claude -p` can start a negotiation but can't finish an unsupervised multi-round one).

## Config Location & Backup
- Live runtime homes (real files, no symlinks, no iCloud): `~/.claude/`, `~/.warp/`, `~/.zshrc`, `~/.config/{git,zed}`, `~/.ssh`.
- **Model setup**: the session `model` and `advisorModel: opus` (built-in advisor, consulted autonomously mid-task) live in `~/.claude/settings.json`. The `agent-roster.sh` hook (SessionStart, PostToolUse[Agent], PostCompact) tracks resumable sub-agent IDs across compaction so a spawned subagent can be resumed via `SendMessage` instead of respawned from scratch.
...
- **Never update a PR branch with GitHub's "Update branch" button in rebase mode, or with `gh pr update-branch --rebase`.** GitHub replays your commits server-side without signing them, so they come back "Unverified" and the committer email is silently swapped for your GitHub noreply address. Update the branch locally instead - `git fetch origin && git rebase origin/main`, which does honour `commit.gpgsign` - then `git push --force-with-lease`. The *merge* mode of the same button is safe, because there GitHub authors a new commit and signs it with its web-flow key.
- **To repair a branch that already came back unsigned**: `git commit --amend --no-edit` re-signs a single commit and leaves its tree untouched (verify with `git diff <old-sha> HEAD`, which must be empty); for a run of commits use `git rebase --exec 'git commit --amend --no-edit -S' origin/main`. Then `git push --force-with-lease`. Check the result with `git log --pretty='%h %G?'` locally, where `G` means good, and `gh api repos/<owner>/<repo>/commits/<sha> --jq .commit.verification` for GitHub's own verdict.
